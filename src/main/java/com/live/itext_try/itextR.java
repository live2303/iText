...
public class itextR {
    ...
    public void generateReport(Map param) throws FileNotFoundException, IOException{
        ...
        canvas.add(new Paragraph().add(new Text("Fecha del evento: ").setFont(bold).setFontSize(12).setFontColor(BMcolor)).add(fecha).add("\n").add(new Text("Responsable de seguimiento: ").setFont(bold).setFontSize(12).setFontColor(BMcolor)).add(responsable).setTextAlignment(TextAlignment.LEFT).setFixedLeading(20f));
        canvas.close();
        ...
        Paragraph oper = new Paragraph(new Text("Operativo ").setFont(bold).setFontColor(param.get("operativo").equals("X") ?  BMcolor : disableColor)).setFontSize(11);
        if(!param.get("operativoAfectacion").equals("")) oper.add(new Text(String.format("(Nivel de afectación: %s)\n",(String)param.get("operativoAfectacion"))).setFont(italic)); else oper.add(noInfo);
        if(!param.get("operativoDetalle").equals("")) oper.add(new Text(String.format(" %s",(String)param.get("operativoDetalle"))).setFont(regular));
        
        Paragraph mon = new Paragraph(new Text("Monetario ").setFont(bold).setFontColor(param.get("monetario").equals("X") ?  BMcolor : disableColor)).setFontSize(11);
        if(!param.get("monetarioAfectacion").equals("")) mon.add(new Text(String.format("(Nivel de afectación: %s)\n",(String)param.get("monetarioAfectacion"))).setFont(italic)); else mon.add(noInfo);
        if(!param.get("monetarioDetalle").equals("")) mon.add(new Text(String.format(" %s",(String)param.get("monetarioDetalle"))).setFont(regular));
        
        Paragraph rep = new Paragraph(new Text("Reputación ").setFont(bold).setFontColor(param.get("reputacion").equals("X") ?  BMcolor : disableColor)).setFontSize(11);
        if(!param.get("reputacionAfectacion").equals("")) rep.add(new Text(String.format("(Nivel de afectación: %s)\n",(String)param.get("reputacionAfectacion"))).setFont(italic)); else rep.add(noInfo);
        if(!param.get("reputacionDetalle").equals("")) rep.add(new Text(String.format(" %s",(String)param.get("reputacionDetalle"))).setFont(regular));
        
        Paragraph salud = new Paragraph(new Text("Salud ").setFont(bold).setFontColor(param.get("salud").equals("X") ?  BMcolor : disableColor)).setFontSize(11);
        if(!param.get("saludAfectacion").equals("")) salud.add(new Text(String.format("(Nivel de afectación: %s)\n",(String)param.get("saludAfectacion"))).setFont(italic)); else salud.add(noInfo);
        if(!param.get("saludDetalle").equals("")) salud.add(new Text(String.format(" %s",(String)param.get("saludDetalle"))).setFont(regular));
        
        Paragraph medA = new Paragraph(new Text("Medio Ambiente ").setFont(bold).setFontColor(param.get("medioAmbiente").equals("X") ?  BMcolor : disableColor)).setFontSize(11).setMarginBottom(15f);
        if(!param.get("medioAfectacion").equals("")) medA.add(new Text(String.format("(Nivel de afectación: %s)\n",(String)param.get("medioAfectacion"))).setFont(italic)); else medA.add(noInfo);
        if(!param.get("medioDetalle").equals("")) medA.add(new Text(String.format(" %s",(String)param.get("medioDetalle"))).setFont(regular));
...
//                table.addCell(new Cell().setFont(regular).add(String.format("Responsable: %s",param.get("responsable"+i))).setTextAlignment(TextAlignment.LEFT).setBorder(Border.NO_BORDER));
//                table.addCell(new Cell().setFont(italic).add(String.format("Fecha: %s",param.get("fecha"+i))).setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER));
                ...
                if(i<maxAccions) document.add( new LineSeparator(line).setWidthPercent(90).setMarginBottom(5f).setMarginTop(5f).setHorizontalAlignment(HorizontalAlignment.CENTER));
            }
        }
        ...
        if(!param.get("fechaSeguimiento").equals("")) tableF.addCell(new Cell().add("Fecha de cierre").setFont(bold).setFontSize(8).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER).setFontColor(BMcolor)); else tableF.addCell(new Cell().setBorder(Border.NO_BORDER));
